...
//  If gcc is not the C compiler, 
//		it switches to abstract control machine
#ifdef __GNUC__
...
int main(int argc, char **argv) {
	if(argc != 2) {
		fprintf(stdout, "Usage: ./%s befunge93_src_code.bf\n", argv[0]);
		exit(0);	
	}

// Jump table with ascii codes of each befunge instrunction.
#ifdef __GNUC__
	...
	signed long int h, g; char ch;
	// Main execution
	// Prefetch next command if pc movement is known
	// It is not known in every instruction(e.g. jumps)
	while(1) {
	next_instruction:
			...
			case _div:
				div_label:
					...
					if(h)
						...
						printf("Exception: Division by zero. Aborting...\n");
						return 1;
					}
					NEXT_INSTRUCTION;
			case _mod:
